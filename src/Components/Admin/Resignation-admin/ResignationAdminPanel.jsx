...
function ResignationAdminPanel() {
  ...
  const handleStatusChange = async (employee, resignationId, newStatus) => {
    try {
      // Update the backend
      await axios.put(`http://server.ovf.bgg.mybluehostin.me:8080/editResignationStatus/${resignationId}`
        , {
        id: resignationId,
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8">
      {/* Header */}
      <div className="mb-6">
        <h2 className="text-xl md:text-2xl font-semibold text-yellow-600">Resignation</h2>
        <div className="text-yellow-600 text-sm md:text-base">
          Dashboard /<span className="text-yellow-600 font-medium">Resignation</span>
        </div>
      </div>
